...
class IO {
public:
	OVERLAPPED overLapped;          //重叠CLIENT_IO ol不用放在首位置也可以找到io地址
	WSABUF wsaBuf;                  //收发数据区
...
struct WOKERTHREADPARA {
	...
	int threadId;
public:
	WOKERTHREADPARA(HANDLE& iocp, SOCKET_HANDLE& server, vector<SOCKET_HANDLE*>& clients,
		LPFN_ACCEPTEX& wsaAccept,LPFN_GETACCEPTEXSOCKADDRS& getClientAddr, int threadId):
		iocp(iocp), server(server), clients(clients), wsaAccept(wsaAccept), getClientAddr(getClientAddr), threadId(threadId) {
	}
